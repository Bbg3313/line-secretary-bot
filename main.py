...
- hospital_name: 아래 목록에서 메시지에 나온 곳만 사용. 없으면 "기타".
  목록: Jy, Clyve, Thebb, our, Delp, EverB, Seoulartline, will, Kleam, Lacela, DerAan, Renovo, Glory, harmony, Mi-k, MYCELL
  별칭 변환: "제이와이"→Jy, "마이셀"→MYCELL, "클라이브"→Clyve, "델프"→Delp, "테브"→Thebb, "에버비"→EverB, "서울아트라인"→Seoulartline, "클램"→Kleam, "라셀라"→Lacela, "레노보"→Renovo, "글로리"→Glory, "하모니"→harmony, "미케이"→Mi-k, "더안"→DerAan
- task_type: 반드시 아래 중 하나만: "광고예산", "콘텐츠", "세금계산서", "CS", "예약", "마케팅", "행정", "미팅", "개인". 모르면 "개인".
...
예시:
{{"tasks":[{{"title":"예산 증액","hospital_name":"MYCELL","task_type":"광고예산","deadline":"{today_ymd}","description":"마이셀 메타 광고 예산 증액"}},{{"title":"세금계산서 발행","hospital_name":"Clyve","task_type":"세금계산서","deadline":null,"description":"Clyve 3월분 세금계산서 발행"}}]}}
...
def _fallback_extract_from_message(text: str) -> dict:
    ...
    return {"hospital_name": hospital, "task_type": "개인", "deadline": deadline}
...
TASK_TYPES_ALLOWED = frozenset({
    "광고예산", "콘텐츠", "세금계산서", "CS", "예약", "마케팅", "행정", "미팅", "개인",
})
...
def handle_message(event: MessageEvent):
    ...
    if not has_valid:
        ...
        tasks = [{
            ...
            "task_type": extracted.get("task_type") or "개인",
...
async def debug_insert():
    ...
    if not has_valid:
        extracted = _fallback_extract_from_message(text)
        tasks = [{"title": summary[:30] or text[:30], "description": text[:200], "hospital_name": extracted.get("hospital_name") or "기타", "task_type": extracted.get("task_type") or "개인", "deadline": extracted.get("deadline")}]
