...
- hospital_name: 아래 목록에서 메시지에 나온 곳만 사용. 없으면 "기타".
  ...
  별칭 변환: "제이와이"→Jy, "마이셀"→MYCELL, "클라이브"→Clyve, "델프"→Delp, "테브"→Thebb, "에버비"→EverB, "서울아트라인"→Seoulartline, "클램"→Kleam, "라셀라"→Lacela, "레노보"→Renovo, "글로리"→Glory, "하모니"→harmony, "미케이"→Mi-k, "더안"→DerAan, "코리안다이어트"→Korean Diet
