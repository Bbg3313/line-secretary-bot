...
**그래도 나간다면**  
- Render 무료 플랜은 15분만 비활성이면 **슬립**합니다. 봇을 그룹에 추가할 때 LINE이 웹훅을 보내는데, 서버가 자고 있으면 요청이 타임아웃되고 LINE이 웹훅 실패로 보고 봇을 퇴장시킬 수 있습니다.  
- **해결**: 그룹에 봇을 넣기 **직전에** 브라우저에서 **서비스 URL**(예: `https://본인서비스.onrender.com/`)을 한 번 열어서 서버를 깨운 뒤, **1분 안에** LINE에서 봇을 그룹에 초대해 보세요.  
- 또는 **슬립 방지**를 켜 두면(아래 UptimeRobot 등) 이런 현상이 거의 없습니다.
